...
class FSMonitor {
    ...
    start() {
        if (!SystemUtils.dirExists(__classPrivateFieldGet(this, _FSMonitor_monitorDir, "f"))) {
            PeppyMeterContext_1.default.getLogger().warn(`[peppymeter] ${__classPrivateFieldGet(this, _FSMonitor_monitorDir, "f")} does not exist. ${this.name} will not start.`);
            return;
        }
        PeppyMeterContext_1.default.getLogger().info(`[peppymeter] ${this.name} commencing initial scanning of ${__classPrivateFieldGet(this, _FSMonitor_monitorDir, "f")}`);
        ...
        __classPrivateFieldGet(this, _FSMonitor_watcher, "f").on('ready', () => {
            PeppyMeterContext_1.default.getLogger().info(`[peppymeter] ${this.name} has completed initial scanning`);
            PeppyMeterContext_1.default.getLogger().info(`[peppymeter] ${this.name} is now watching ${__classPrivateFieldGet(this, _FSMonitor_monitorDir, "f")}`);
    ...
    async stop() {
        ...
        __classPrivateFieldSet(this, _FSMonitor_status, 'stopped', "f");
        PeppyMeterContext_1.default.getLogger().warn(`[peppymeter] ${this.name} stopped`);
    }
    get status() {
