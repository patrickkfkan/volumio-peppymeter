...
class ImageTitleFactory():
    ...
    # --- volumio-peppymeter -------
    # strip unicode characters above 0xFFFF
    # On pygame pre-2.0.3, font render fails when string contains UCS4 chars ("A Unicode character above '\uFFFF' was found; not supported").
    # Installing pygame 2.x through pip is possible, but performnace is worse at least on rPi due to lack of 
    # CPU optimizations. On the other hand, compiling with CPU optimizations somehow results in pygame crashing on rPi.
    # Furthermore, if we were to compile pygame, we would have to make sure it will work across different archs with the appropriate
    # optimizations...Ugh.
    # So best to leave current version 1.9.4 as is, and strip the unsupported characters out of the string (most likely to be
    # text emojis anyway).
    def strip_ucs4(self, txt):
        f = filter(lambda c: ord(c) <= 0xFFFF, txt)
        return "".join(f)
